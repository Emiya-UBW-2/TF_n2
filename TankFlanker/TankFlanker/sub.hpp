								...
								//ダメージ面に当たった時に装甲値に勝てるかどうか
								{
									...
									if (c.spec.pene_a > a.second * (1.0f / std::abs(vec_t.Norm().dot(normal)))) {
										...
										if (veh_t.HP == 0 && ttn != veh_t.HP) {
											...
											t.effcs[ef_bomb].set(veh_t.obj.frame(veh_t.use_veh.gunframe[0].frame_turn.first), VGet(0, 0, 0));
										}
										//弾処理
										c.hit = true;
										c.flug = false;
		...
		void set_alive(float& se_vol) {
			...
			if (veh.HP == 0) {
				if (GetRand(100) < 12) {
					...
					if (veh.HP_m[tmp] != 0) {
						this->effcs[ef_bomb].set(veh.obj.frame(veh.use_veh.gunframe[0].frame_turn.first), VGet(0, 0, 0), 0.5f);
					}
					veh.HP_m[tmp] = 0;
				}
			}
