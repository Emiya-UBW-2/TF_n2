...
/**/
		{
				...
				//描画
				{
					...
					if (Drawparts->use_vr) {
						...
						//cam_s.cam
						{
							...
							{
								cam_s.cam.campos = veh.obj.frame(veh.use_veh.fps_view.first) + MATRIX_ref::Vtrans(/*eye_pos_ads*/ VGet(0, 0.58f, 0), veh.mat);
			...
			//保存
			{
				...
				while (true) {
					{
						fout.write((char *)&tt->speed, sizeof(tt->speed));
						fout.write((char *)&tt->v_mat, sizeof(tt->v_mat));
						fout.write((char *)&tt->mat, sizeof(tt->mat));
						fout.write((char *)&tt->pos, sizeof(tt->pos));
						for (auto& p : tt->Gun_) {
							for (auto& a : p.bullet) {
						...
						for (int i = 0; i < ef_size; i++) {
							fout.write((char *)&tt->effcs_[i].flug, sizeof(tt->effcs_[i].flug));
							fout.write((char *)&tt->effcs_[i].pos, sizeof(tt->effcs_[i].pos));
							fout.write((char *)&tt->effcs_[i].nor, sizeof(tt->effcs_[i].nor));
							fout.write((char *)&tt->effcs_[i].scale, sizeof(tt->effcs_[i].scale));
						}
						for (int i = 0; i < 8; i++) {
							fout.write((char *)&tt->effcs_missile_[i].first.flug, sizeof(tt->effcs_missile_[i].first.flug));
							fout.write((char *)&tt->effcs_missile_[i].first.pos, sizeof(tt->effcs_missile_[i].first.pos));
							fout.write((char *)&tt->effcs_missile_[i].first.nor, sizeof(tt->effcs_missile_[i].first.nor));
							fout.write((char *)&tt->effcs_missile_[i].first.scale, sizeof(tt->effcs_missile_[i].first.scale));
							fout.write((char *)&tt->effcs_missile_[i].flug, sizeof(tt->effcs_missile_[i].flug));
							fout.write((char *)&tt->effcs_missile_[i].n_l, sizeof(tt->effcs_missile_[i].n_l));
							fout.write((char *)&tt->effcs_missile_[i].pos, sizeof(tt->effcs_missile_[i].pos));
							fout.write((char *)&tt->effcs_missile_[i].cnt, sizeof(tt->effcs_missile_[i].cnt));
						}
						for (int i = 0; i < 12; i++) {
							fout.write((char *)&tt->effcs_gun_[i].first.flug, sizeof(tt->effcs_gun_[i].first.flug));
							fout.write((char *)&tt->effcs_gun_[i].first.pos, sizeof(tt->effcs_gun_[i].first.pos));
							fout.write((char *)&tt->effcs_gun_[i].first.nor, sizeof(tt->effcs_gun_[i].first.nor));
							fout.write((char *)&tt->effcs_gun_[i].first.scale, sizeof(tt->effcs_gun_[i].first.scale));
							fout.write((char *)&tt->effcs_gun_[i].flug, sizeof(tt->effcs_gun_[i].flug));
							fout.write((char *)&tt->effcs_gun_[i].n_l, sizeof(tt->effcs_gun_[i].n_l));
							fout.write((char *)&tt->effcs_gun_[i].pos, sizeof(tt->effcs_gun_[i].pos));
							fout.write((char *)&tt->effcs_gun_[i].cnt, sizeof(tt->effcs_gun_[i].cnt));
						}
						for (int i = 0; i < 3; i++) {
							fout.write((char *)&tt->gndsmkeffcs_[i], sizeof(tt->gndsmkeffcs_[i]));
						}
					}
					{
						fout.write((char *)&tt2->speed, sizeof(tt2->speed));
						fout.write((char *)&tt2->v_mat, sizeof(tt2->v_mat));
						fout.write((char *)&tt2->mat, sizeof(tt2->mat));
						fout.write((char *)&tt2->pos, sizeof(tt2->pos));
						for (auto& p : tt2->Gun_) {
							for (auto& a : p.bullet) {
								fout.write((char *)&a.flug, sizeof(a.flug));
								fout.write((char *)&a.pos, sizeof(a.pos));
						...
						for (int i = 0; i < ef_size; i++) {
							fout.write((char *)&tt2->effcs_[i].flug, sizeof(tt2->effcs_[i].flug));
							fout.write((char *)&tt2->effcs_[i].pos, sizeof(tt2->effcs_[i].pos));
							fout.write((char *)&tt2->effcs_[i].nor, sizeof(tt2->effcs_[i].nor));
							fout.write((char *)&tt2->effcs_[i].scale, sizeof(tt2->effcs_[i].scale));
						}
						for (int i = 0; i < 8; i++) {
							fout.write((char *)&tt2->effcs_missile_[i].first.flug, sizeof(tt2->effcs_missile_[i].first.flug));
							fout.write((char *)&tt2->effcs_missile_[i].first.pos, sizeof(tt2->effcs_missile_[i].first.pos));
							fout.write((char *)&tt2->effcs_missile_[i].first.nor, sizeof(tt2->effcs_missile_[i].first.nor));
							fout.write((char *)&tt2->effcs_missile_[i].first.scale, sizeof(tt2->effcs_missile_[i].first.scale));
							fout.write((char *)&tt2->effcs_missile_[i].flug, sizeof(tt2->effcs_missile_[i].flug));
							fout.write((char *)&tt2->effcs_missile_[i].n_l, sizeof(tt2->effcs_missile_[i].n_l));
							fout.write((char *)&tt2->effcs_missile_[i].pos, sizeof(tt2->effcs_missile_[i].pos));
							fout.write((char *)&tt2->effcs_missile_[i].cnt, sizeof(tt2->effcs_missile_[i].cnt));
						}
						for (int i = 0; i < 12; i++) {
							fout.write((char *)&tt2->effcs_gun_[i].first.flug, sizeof(tt2->effcs_gun_[i].first.flug));
							fout.write((char *)&tt2->effcs_gun_[i].first.pos, sizeof(tt2->effcs_gun_[i].first.pos));
							fout.write((char *)&tt2->effcs_gun_[i].first.nor, sizeof(tt2->effcs_gun_[i].first.nor));
							fout.write((char *)&tt2->effcs_gun_[i].first.scale, sizeof(tt2->effcs_gun_[i].first.scale));
							fout.write((char *)&tt2->effcs_gun_[i].flug, sizeof(tt2->effcs_gun_[i].flug));
							fout.write((char *)&tt2->effcs_gun_[i].n_l, sizeof(tt2->effcs_gun_[i].n_l));
							fout.write((char *)&tt2->effcs_gun_[i].pos, sizeof(tt2->effcs_gun_[i].pos));
							fout.write((char *)&tt2->effcs_gun_[i].cnt, sizeof(tt2->effcs_gun_[i].cnt));
						}
						for (int i = 0; i < 3; i++) {
							fout.write((char *)&tt2->gndsmkeffcs_[i], sizeof(tt2->gndsmkeffcs_[i]));
						}
					}
					{
...
//*
		{
			...
			while (true) {
				{
					fout.read((char *)&tt->speed, sizeof(tt->speed));
					fout.read((char *)&tt->v_mat, sizeof(tt->v_mat));
					fout.read((char *)&tt->mat, sizeof(tt->mat));
					fout.read((char *)&tt->pos, sizeof(tt->pos));
					tt->Gun_.clear();
					for (int i = 0; i < 5; i++) {
						tt->Gun_.resize(tt->Gun_.size() + 1);
						auto& p = tt->Gun_.back();
						for (auto& a : p.bullet) {
					...
					for (int i = 0; i < ef_size; i++) {
						fout.read((char *)&tt->effcs_[i].flug, sizeof(tt->effcs_[i].flug));
						fout.read((char *)&tt->effcs_[i].pos, sizeof(tt->effcs_[i].pos));
						fout.read((char *)&tt->effcs_[i].nor, sizeof(tt->effcs_[i].nor));
						fout.read((char *)&tt->effcs_[i].scale, sizeof(tt->effcs_[i].scale));
					}
					for (int i = 0; i < 8; i++) {
						fout.read((char *)&tt->effcs_missile_[i].first.flug, sizeof(tt->effcs_missile_[i].first.flug));
						fout.read((char *)&tt->effcs_missile_[i].first.pos, sizeof(tt->effcs_missile_[i].first.pos));
						fout.read((char *)&tt->effcs_missile_[i].first.nor, sizeof(tt->effcs_missile_[i].first.nor));
						fout.read((char *)&tt->effcs_missile_[i].first.scale, sizeof(tt->effcs_missile_[i].first.scale));
						fout.read((char *)&tt->effcs_missile_[i].flug, sizeof(tt->effcs_missile_[i].flug));
						fout.read((char *)&tt->effcs_missile_[i].n_l, sizeof(tt->effcs_missile_[i].n_l));
						fout.read((char *)&tt->effcs_missile_[i].pos, sizeof(tt->effcs_missile_[i].pos));
						fout.read((char *)&tt->effcs_missile_[i].cnt, sizeof(tt->effcs_missile_[i].cnt));
					}
					for (int i = 0; i < 12; i++) {
						fout.read((char *)&tt->effcs_gun_[i].first.flug, sizeof(tt->effcs_gun_[i].first.flug));
						fout.read((char *)&tt->effcs_gun_[i].first.pos, sizeof(tt->effcs_gun_[i].first.pos));
						fout.read((char *)&tt->effcs_gun_[i].first.nor, sizeof(tt->effcs_gun_[i].first.nor));
						fout.read((char *)&tt->effcs_gun_[i].first.scale, sizeof(tt->effcs_gun_[i].first.scale));
						fout.read((char *)&tt->effcs_gun_[i].flug, sizeof(tt->effcs_gun_[i].flug));
						fout.read((char *)&tt->effcs_gun_[i].n_l, sizeof(tt->effcs_gun_[i].n_l));
						fout.read((char *)&tt->effcs_gun_[i].pos, sizeof(tt->effcs_gun_[i].pos));
						fout.read((char *)&tt->effcs_gun_[i].cnt, sizeof(tt->effcs_gun_[i].cnt));
					}
					for (int i = 0; i < 3; i++) {
						fout.read((char *)&tt->gndsmkeffcs_[i], sizeof(tt->gndsmkeffcs_[i]));
					}
				}
				{
					fout.read((char *)&tt2->speed, sizeof(tt2->speed));
					fout.read((char *)&tt2->v_mat, sizeof(tt2->v_mat));
					fout.read((char *)&tt2->mat, sizeof(tt2->mat));
					fout.read((char *)&tt2->pos, sizeof(tt2->pos));
					tt2->Gun_.clear();
					for (int i = 0; i < 5; i++) {
						tt2->Gun_.resize(tt2->Gun_.size() + 1);
						auto& p = tt2->Gun_.back();
						for (auto& a : p.bullet) {
							fout.read((char *)&a.flug, sizeof(a.flug));
							fout.read((char *)&a.pos, sizeof(a.pos));
							fout.read((char *)&a.repos, sizeof(a.repos));
					...
					for (int i = 0; i < ef_size; i++) {
						fout.read((char *)&tt2->effcs_[i].flug, sizeof(tt2->effcs_[i].flug));
						fout.read((char *)&tt2->effcs_[i].pos, sizeof(tt2->effcs_[i].pos));
						fout.read((char *)&tt2->effcs_[i].nor, sizeof(tt2->effcs_[i].nor));
						fout.read((char *)&tt2->effcs_[i].scale, sizeof(tt2->effcs_[i].scale));
					}
					for (int i = 0; i < 8; i++) {
						fout.read((char *)&tt2->effcs_missile_[i].first.flug, sizeof(tt2->effcs_missile_[i].first.flug));
						fout.read((char *)&tt2->effcs_missile_[i].first.pos, sizeof(tt2->effcs_missile_[i].first.pos));
						fout.read((char *)&tt2->effcs_missile_[i].first.nor, sizeof(tt2->effcs_missile_[i].first.nor));
						fout.read((char *)&tt2->effcs_missile_[i].first.scale, sizeof(tt2->effcs_missile_[i].first.scale));
						fout.read((char *)&tt2->effcs_missile_[i].flug, sizeof(tt2->effcs_missile_[i].flug));
						fout.read((char *)&tt2->effcs_missile_[i].n_l, sizeof(tt2->effcs_missile_[i].n_l));
						fout.read((char *)&tt2->effcs_missile_[i].pos, sizeof(tt2->effcs_missile_[i].pos));
						fout.read((char *)&tt2->effcs_missile_[i].cnt, sizeof(tt2->effcs_missile_[i].cnt));
					}
					for (int i = 0; i < 12; i++) {
						fout.read((char *)&tt2->effcs_gun_[i].first.flug, sizeof(tt2->effcs_gun_[i].first.flug));
						fout.read((char *)&tt2->effcs_gun_[i].first.pos, sizeof(tt2->effcs_gun_[i].first.pos));
						fout.read((char *)&tt2->effcs_gun_[i].first.nor, sizeof(tt2->effcs_gun_[i].first.nor));
						fout.read((char *)&tt2->effcs_gun_[i].first.scale, sizeof(tt2->effcs_gun_[i].first.scale));
						fout.read((char *)&tt2->effcs_gun_[i].flug, sizeof(tt2->effcs_gun_[i].flug));
						fout.read((char *)&tt2->effcs_gun_[i].n_l, sizeof(tt2->effcs_gun_[i].n_l));
						fout.read((char *)&tt2->effcs_gun_[i].pos, sizeof(tt2->effcs_gun_[i].pos));
						fout.read((char *)&tt2->effcs_gun_[i].cnt, sizeof(tt2->effcs_gun_[i].cnt));
					}
					for (int i = 0; i < 3; i++) {
						fout.read((char *)&tt2->gndsmkeffcs_[i], sizeof(tt2->gndsmkeffcs_[i]));
					}
				}
				{
